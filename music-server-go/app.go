...
func (a *App) startup(ctx context.Context) {
	...
	a.wsManager.onStatusChange = func(ip, state, file string) {
		if state == "playing" {
			a.tracker.SetPlaying(ip, file)
		} else if state == "stopped" || state == "disconnected" {
			a.tracker.SetStopped(ip)
		} else if state == "connected" {
			a.tracker.SetConnected(ip)
		}
		runtime.EventsEmit(ctx, "ws-update", map[string]string{
			"ip": ip, "state": state, "file": file,
		})
	}
	a.initMusicDir()
	a.initDeviceNames()
...
func (a *App) shutdown(_ context.Context) {
	if a.server != nil {
		...
		defer cancel()
		a.server.Shutdown(sdCtx)
	}
}

func (a *App) initDeviceNames() {
	exe, err := os.Executable()
	if err == nil {
		a.namesPath = filepath.Join(filepath.Dir(exe), "device-names.json")
	} else {
		a.namesPath = "device-names.json"
	}
	data, err := os.ReadFile(a.namesPath)
	if err != nil {
		return
	}
	json.Unmarshal(data, &a.deviceNames)
}

func (a *App) saveDeviceNames() {
	data, _ := json.MarshalIndent(a.deviceNames, "", "  ")
	os.WriteFile(a.namesPath, data, 0644)
}

func (a *App) GetDeviceNames() map[string]string {
	cp := make(map[string]string, len(a.deviceNames))
...
func (a *App) SetDeviceName(ip, name string) string {
	a.deviceNames[ip] = name
	a.saveDeviceNames()
	return "ok"
}

func (a *App) GetDeviceModes() map[string]string {
	cp := make(map[string]string, len(a.deviceModes))
	for k, v := range a.deviceModes {
		cp[k] = v
	}
	return cp
}

func (a *App) SetDeviceMode(ip, mode string) string {
	a.deviceModes[ip] = mode
	err := a.wsManager.SendCommandWithValue(ip, "mode", mode)
	if err != nil {
		return err.Error()
	}
	return "ok"
...
func (a *App) startHTTPServer() {
	...
	mux.HandleFunc("/api/clients", a.handleAPIClients)
	mux.HandleFunc("/api/play", a.handleAPIPlay)
	mux.HandleFunc("/api/stop", a.handleAPIStop)
	mux.HandleFunc("/api/files", a.handleAPIFiles)
	mux.HandleFunc("/api/localip", a.handleAPILocalIP)
	mux.HandleFunc("/api/play-all", a.handleAPIForwardPlay)
	mux.HandleFunc("/api/stop-all", a.handleAPIForwardStop)
	mux.HandleFunc("/api/names", a.handleAPINames)
	mux.HandleFunc("/api/name", a.handleAPINameSet)
	mux.HandleFunc("/api/modes", a.handleAPIModes)
	mux.HandleFunc("/api/mode", a.handleAPIModeSet)
	mux.HandleFunc("/app/", a.handleWebApp)
	mux.HandleFunc("/", a.handleWebApp)
